...
class ImagePreprocessor:
    ...
    def detect_shadows(self, image: np.ndarray) -> np.ndarray:
        """Detect shadow regions using HSV color space and morphological operations
        ...
        # Shadows typically have V < 100 and S < 80
    ...
    def remove_shadows(self, image: np.ndarray, shadow_mask: np.ndarray) -> np.ndarray:
        """Remove shadows while preserving the original colors (especially magenta)
        ...
        # Convert to LAB color space for better illumination correction
        lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        l_channel, a_channel, b_channel = cv2.split(lab)
        
        # Apply illumination correction only to shadow regions
        # Increase the L channel in shadow areas
        ...
        # CRITICAL: Preserve magenta phenophthalein regions
        # Detect magenta regions (high red, low green, high blue)
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
